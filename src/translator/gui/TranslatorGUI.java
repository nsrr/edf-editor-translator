...
public class TranslatorGUI extends JPanel implements ActionListener,
    ItemListener {
  ...
  public TranslatorGUI() {
    ...
    String[] vendorList = new String[] {
        ...
        Vendor.Embla_EDFBrowser.toString(), // Add this line for translating
                                            // annotation to EDFBrowser version;
                                            // TODO
        Vendor.Compumedics_EDFBrowser.toString(), // Add this line for
                                                  // translating annotation to
                                                  // EDFBrowser version; TODO
        Vendor.ARES.toString(),
        Vendor.Respironics.toString(), Vendor.Sandman.toString() };
    JComboBox<String> JComboBox_vendor = new JComboBox<String>(vendorList);
    JComboBox_vendor.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        String example = AnnotationTranslatorClient.customize_out_file(pattern,
            null, vendor_Selected);
        JLabel_OutputExample.setText("<html><font color='blue'>" + example
            + "</font></html>");
      }
    });
    ...
    LayoutManager.addFirstField(new JLabel(
        "Select input-directory of EDF files:"), this);
    ...
    LayoutManager.addFirstField(new JLabel(
        "Select input-directory of annotation files:"), this);
    ...
    LayoutManager.addFirstField(new JLabel(
        "Select input-directory of stage files:"), this);
    ...
    LayoutManager.addFirstField(new JLabel(
        "Select output-directory of translated files:"), this);
    ...
    Checkbox_Vendor.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        String pattern = AnnotationTranslatorClient.updateOutputPattern(
            JTextField_OutputPattern.getText(), Keywords.key_vendor, e);
        JTextField_OutputPattern.setText(pattern);
      }
    });
    Checkbox_Date.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        String pattern = AnnotationTranslatorClient.updateOutputPattern(
            JTextField_OutputPattern.getText(), Keywords.key_date, e);
        JTextField_OutputPattern.setText(pattern);
      }
    });
    Checkbox_Time.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        String pattern = AnnotationTranslatorClient.updateOutputPattern(
            JTextField_OutputPattern.getText(), Keywords.key_time, e);
        JTextField_OutputPattern.setText(pattern);
      }
    });
    ...
    JTextField_OutputPattern.getDocument().addDocumentListener(
        new DocumentListener() {
          ...
          private void update() {
            String pattern = JTextField_OutputPattern.getText();
            String example = AnnotationTranslatorClient.customize_out_file(
                pattern, null, vendor_Selected);
            JLabel_OutputExample.setText("<html><font color='blue'>" + example
                + "</font></html>");
          }
        });
    ...
    LayoutManager.addFirstField(new JLabel("List of EDF files:"),
        JPanel_TwoLists);
    JList_Edf_files = new JList<String>();
    JList_Edf_files
        .setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    ListSelectionModel listSelectionModel1 = JList_Edf_files
        .getSelectionModel();
    listSelectionModel1.addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent e) {
        if (e.getValueIsAdjusting()) {
          ...
          Object[] selected_Objects = JList_Edf_files.getSelectedValues(); // this
                                                                           // should
                                                                           // change
                                                                           // to
    ...
    LayoutManager.addFirstField(new JLabel("List of Output files:"),
        JPanel_TwoLists);
    JList_Out_files = new JList<String>();
    JList_Out_files.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    ListSelectionModel listSelectionModel2 = JList_Out_files
        .getSelectionModel();
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    select
        .setCurrentDirectory(Keywords.fileFolderChooser.getCurrentDirectory());
    ...
    if (e.getSource() == JButton_MappingFile) {
      ...
      FileNameExtensionFilter filter = new FileNameExtensionFilter(
          "Mapping file (*.csv)", new String[] { "CSV", "csv", "Csv" });
      ...
      if (select.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
        ...
        Collection<File> fileCollection = FileUtils.listFiles(
            new File(pathname), new String[] { "edf", "Edf", "eDf", "edF",
                "EDf", "EdF", "eDF", "EDF" }, true);
        ...
        if (JTextField_StageDirectory.getText().equals("")) {
          if (vendor_Selected != null
              && vendor_Selected == Vendor.Respironics.toString()) {
      ...
      AnnotationTranslatorClient
          .addElementIntoLog(
              "=====================================================================",
              true);
      AnnotationTranslatorClient.addElementIntoLog(
          " + User started a new task at " + MyDate.currentDateTime(), true);

      ArrayList<String> errorMessages = validatePrerequisites();

      if (errorMessages.size() > 0) {
        for (String message : errorMessages) {
          AnnotationTranslatorClient.addElementIntoLog("    * " + message,
              false);
        }
        AnnotationTranslatorClient
            .addElementIntoLog(
                " + The task failed. Please check the above error messages!",
                false);
      } else {
        ...
        ArrayList<String> successfulOutAL = AnnotationTranslatorClient
            .conductTranslation(vendor, mapping_file, edf_dir,
                selected_Edf_files, annotation_dir, stage_dir, output_dir,
                outname);

        String[] outputFiles = successfulOutAL
            .toArray(new String[successfulOutAL.size()]);
  ...
  private ArrayList<String> validatePrerequisites() {
    ...
    if (annotation_dir.equals(""))
      errorMessages
          .add("Please select the input-directory of Annotation files;");
    if (vendor_Selected == Vendor.Respironics.toString()
        && stage_dir.equals(""))
      errorMessages.add("Please select the input-directory of Stage files;");
    if (output_dir.equals(""))
      errorMessages
          .add("Please select the output-directory of resulting files;");
    ...
    if (!edf_dir.equals("") && !File_EdfDirectory.exists())
      errorMessages
          .add("The chosen input-directory of EDF files no longer exists;");
    if (!annotation_dir.equals("") && !File_AnnotationDirectory.exists())
      errorMessages
          .add("The chosen input-directory of Annotation files no longer exists;");
    if (vendor_Selected == Vendor.Respironics.toString()
        && !stage_dir.equals("") && !File_StageDirectory.exists())
      errorMessages
          .add("The chosen input-directory of Stage files no longer exists;");
    if (!output_dir.equals("") && !File_OutputDirectory.exists())
      errorMessages
          .add("The chosen output-directory of Resulting files no longer exists;");
   ...
   * 
   * @param enable
   *          true to enable specified components related to the selected vendor
   */
