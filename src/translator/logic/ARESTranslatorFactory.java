...
public class ARESTranslatorFactory extends AbstractTranslatorFactory {
	...
	private Element parseARESxmlEvent(Element scoredEventElement) {
		...
		if (subattribute == null || subattribute == ""
		...
		if (attribute != null && map[1].keySet().contains(eventKey)) {
			...
			eventConcept.appendChild(xmlRoot
					.createTextNode("Technician Notes|Technician Notes"));
			notesElement.appendChild(xmlRoot.createTextNode(code));
			eventCategory.appendChild(xmlRoot
					.createTextNode("Technician Notes|Technician Notes"));
	...
	private List<Element> getLocation(Element scoredEventElement) {
		...
		// System.out.println("KEY: " + eventKey + "map contains key? " +
		// map[1].containsKey(eventKey));
		...
		String cddname = (String) ((ArrayList<String>) map[1].get(eventKey))
				.get(1);
		@SuppressWarnings("unchecked")
		String aresname = (String) ((ArrayList<String>) map[1].get(eventKey))
				.get(0);
		Node nameNode = xmlRoot.createTextNode(cddname + "|" + aresname);
		String categoryStr = map[3].get(eventKey) == null ? ""
				: (String) map[3].get(eventKey);
	...
	public String getSignalLocationFromEvent(Element scoredEvent,
		...
		String subattributeStr = getElementByChildTag(scoredEvent,
				"subattribute");
	...
	private String getDuration(Element scoredEvent) {
		...
		// System.out.print("Start Time: " + startTime + ", Stop Time:" +
		// stopTime + " ");
	...
	public boolean write2xml(String outputFile) {
		System.out
				.println("   >>> Inside ARESAnnotationTranslator write"); // test
		output = outputFile;
		try {
	...
	/**
	 * Reads mapping file and saves as an array of HashMap Can be put in a
	 * higher hierarchy.
	 * 
	 * @param mapFile
		...
		try {
			...
			try {
				...
				while ((line = input.readLine()) != null) {
					...
					AresItemKey aresKey = new AresItemKey(code, attribute,
							subattribute);
					...
					if (!eventTypeLowerCase.contains("epochlength")) {
						...
						// values = {aresName: String, cddName: String, note:
						// String}
